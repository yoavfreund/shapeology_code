...
def distance_matrix(nodes,data):
    graph = []
    for i in range(len(nodes)):
        node = nodes[i]
        dists=[dist2(node,r) for r in data]
        graph.append(dists)
...
class Kernel(object):
    ...
    def compute(self, Y=None, return_bandwidths=False):
        ...
        if np.array_equal(Y, self.data):  # Avoid recomputing nearest neighbors unless needed.
            ...
            if self.metric == 'precomputed':
                print('recomputing nearest neighbors')
                Y = distance_matrix(Y, self.data)
