...
if __name__=='__main__':
    ...
    savepath = 'CSHL_shift_scores/cdf_search_2D/' + stack + '/'
    ...
    # step_size = max(round(len_max / 30), round(20 / resol))
    ...
    for k in range(-half, half + 1):
        ...
        for i in range(-half, half + 1):
            for j in range(-half, half + 1):
                ...
                # vectors_input = []
                for sec in seq:
                    if sec - seq[0] not in vectors_input.keys():
                        vectors_input[sec - seq[0]] = []
                        loc_input[sec - seq[0]] = []
                    ...
                    if inside_features.shape[0] and outside_features.shape[0]:
                        ...
                        vectors_input[sec - seq[0]].append(feature_vector)
                        loc_input[sec - seq[0]].append(np.array([j + half, i + half, k + half]))
    ...
    start_time = time()
    for index in vectors_input.keys():
        xtest = xgb.DMatrix(vectors_input[index])
        score = bst.predict(xtest, output_margin=True, ntree_limit=bst.best_ntree_limit)
        locs = np.stack(loc_input[index])
        xyz_shift_map[locs[:, 0], locs[:, 1], locs[:, 2]] += score
    time_count('Compute xgboost', time() - start_time)
