...
def collect_inside_cell_features(loc):
    ...
    shape_features = features_in_box[values == 1]
...
if __name__=='__main__':
    ...
    resol = 0.325
    fn = os.environ['ROOT_DIR'] + 'Detection_preparation_v3/' + structure+'.pkl'
    ...
    [left, right, up, down] = [int(max(min(polygon_set[:, 0]) - half * step_size, 0)),
                               int(np.ceil(max(polygon_set[:, 0])  + half * step_size)),
                               int(max(min(polygon_set[:, 1])  - half * step_size, 0)),
                               int(np.ceil(max(polygon_set[:, 1]) + half * step_size))]
    ...
    for k in range(-half, half + 1):
        for i in range(-half, half + 1):
            for j in range(-half, half + 1):
                ...
                loc = np.array([seq[0] + k * step_z, center[0] + min_x + i * step_size, center[1] + min_y + j * step_size])
                shape_features = collect_inside_cell_features(loc)
    ...
    save_dir = os.environ['ROOT_DIR'] + 'CSHL_patch_samples_features_v3/' + stack + '/' + rname
